...
test("observability check validates files, scripts, runbook, and metric hooks", () => {
  withFixture((root) => {
    ...
    writeFixtureFile(
      ...
      "/api/health /api/ready /api/metrics /api/ops/smoke /api/ops/storage-smoke METRICS_ENABLED=true METRICS_TOKEN 5xx p95 500 ms arbor_http_requests_total arbor_http_duration_seconds_bucket arbor_db_pool_waiting restore:db:check smoke:p95 npm run smoke:p95 -- https://<arbor-os-url> --threshold 500 --samples 5 GO NO-GO",
    );
