...
const dryFileCount = new Set(files.filter((f) => report.size && /#[0-9a-fA-F]{6}/.test(fs.readFileSync(f, 'utf8')))).size;
console.log(`${write ? 'ZAPISANO' : 'DRY RUN'} — plików ${write ? 'zmienionych' : 'do zmiany'}: ${write ? changedFiles : (rows.length ? dryFileCount : 0)}, unikalnych obcych hexów: ${rows.length}`);
