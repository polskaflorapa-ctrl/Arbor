...
const { verifySmsStatusWebhookSignatureAsync: verifyZadarmaSmsStatusSignature } = require('../services/zadarma');
...
async function handleZadarmaStatus(req, res) {
  try {
    if (!(await verifyZadarmaSmsStatusSignature(req.body || {}, req.get('x-zadarma-signature') || req.get('signature')))) {
