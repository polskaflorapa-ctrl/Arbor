...
const CRM_STAGES = ['Lead', 'Oferta', 'W realizacji', 'Wygrane', 'Przegrane', 'Techniczny'];
...
function canSeeAllBranches(user) {
  return isDyrektor(user) || isSalesDirector(user);
}

function scopedOddzialId(user, requestedOddzialId) {
  return canSeeAllBranches(user) ? requestedOddzialId : toInt(user?.oddzial_id);
}

function canAccessOddzial(user, oddzialId) {
  return canSeeAllBranches(user) || Number(user?.oddzial_id) === Number(oddzialId);
}
