...
describe('Flota rezerwacje sprzetu', () => {
  ...
  it('PUT status applies branch filter for Administrator', async () => {
    pool.query.mockResolvedValueOnce({ rows: [{ id: 5 }], rowCount: 1 });
    const res = await request(app)
      .put('/api/flota/rezerwacje/5/status')
    ...
    expect(res.status).toBe(200);
    expect(pool.query).toHaveBeenCalledWith(expect.stringContaining('AND e.oddzial_id = $3'), ['Anulowane', 5, 1]);
  });
});
