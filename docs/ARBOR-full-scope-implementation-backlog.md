...
- [x] **P0 pilot execution evidence template**: `docs/PILOT-GO-NO-GO-DECISION-TEMPLATE.md` daje formularz wynikow smoke, manualnego A-Z, wyjatkow, ownerow i podpisanej decyzji; `npm run pilot:run:new -- --date YYYY-MM-DD` tworzy raport w `docs/pilot-runs/`, a `npm run verify:pilot-execution` pilnuje artefaktu.
...
- [x] **6.4** Integracja z raportem zużycia z mobilki (EPIC 2). Finish z `material_id` ksieguje rozchod w `inventory_materials`/`inventory_movements`, zwraca `stan_magazynu_za_maly` przy braku stanu, a web/mobile potrafia przeniesc `material_id`; kontrakt pilnuje `docs/MOBILE-INVENTORY-USAGE-CONTRACT.md` + `verify:mobile-inventory-usage`.
...
- [x] **7.4** Integracja z dispatcherm: EPIC 1.5 + EPIC 7.3 są spójne. Solver dostaje tylko aktywne kompetencje ekip, a `POST /api/dispatch/apply/:id` przed `UPDATE tasks` blokuje zapisany plan przez `TEAM_COMPETENCY_BLOCKED`, jesli aktualny sklad ekipy nie pokrywa `tasks.wymagane_kompetencje`.
