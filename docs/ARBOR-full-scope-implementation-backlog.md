...
- [x] **P0 fleet repair due controls**: `docs/FLEET-REPAIR-DUE-CONTROLS-CONTRACT.md` opisuje terminy odbioru i priorytety napraw; Flota pokazuje filtr `Po terminie`, CSV z `termin_status`, a `npm run verify:fleet-repair-due-controls` pilnuje API, migracji, UI i testow.
- [x] **P0 fleet repair parts cost**: `docs/FLEET-REPAIR-PARTS-COST-CONTRACT.md` opisuje osobne liczenie kosztow czesci napraw; API zwraca `czesci_kwota`/`czesci_count`, Flota dolicza je w UI/CSV, a `npm run verify:fleet-repair-parts-cost` pilnuje, ze dodanie czesci nie mutuje `repair.koszt`.
...
- [x] **P0 ops owner remediation closure**: `/api/ops/owner-alerts/resolve` zapisuje follow-up `risk_owner_resolve`, przyciski `Oznacz rozwiazane` dzialaja z Kontroli operacyjnej i szczegolow digestu, a digest usuwa domkniete alerty z `owner_unresolved_after_remediation`.
