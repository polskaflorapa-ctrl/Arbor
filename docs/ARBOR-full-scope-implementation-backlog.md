...
- [x] **P0 fleet repair due controls**: `docs/FLEET-REPAIR-DUE-CONTROLS-CONTRACT.md` opisuje terminy odbioru, priorytet, status `Po terminie`, przestoj i eksport CSV napraw floty; `npm run verify:fleet-repair-due-controls` pilnuje API, UI, testow i backlogu.
- [x] **P0 fleet repair parts cost**: `docs/FLEET-REPAIR-PARTS-COST-CONTRACT.md` opisuje osobne liczenie kosztow czesci napraw floty bez mutowania bazowego kosztu naprawy; `npm run verify:fleet-repair-parts-cost` pilnuje API, UI i backlogu.
