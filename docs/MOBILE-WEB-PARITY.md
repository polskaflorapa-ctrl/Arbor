...
| `(tabs)/explore.tsx`                    | `#/eksploruj` (`Eksploruj.js`)                             | **Parity**            | Siatka kafelków-skrótów do wszystkich modułów systemu (pogrupowane: Operacje, Pole, Sprzedaż, Zasoby, Finanse, Kadry); wyszukiwarka inline; filtr po roli; dostępna z Sidebara (Start → Eksploruj). |
...
| `rozliczenia.tsx`                       | `#/wynagrodzenie-wyceniajacych`, `#/rozliczenia-ekip`, `#/rozliczenia-polowe` | **Parity** | Kierownicze (`PayrollM11.js`): eksport CSV/Symfonia/Optima/Comarch, korekty, akrual wyceniających. Polowe (`RozliczeniaFieldEntry.js`): godziny pomocników per task_id (upsert `task_pomocnik_godziny`), zatwierdź/odrzuć, kalkulator brutto/VAT → netto → wynagrodzenie brygadzisty, podsumowanie dnia. OS backend odblokowany (`blockPayrollSettlements` usunięte z routera). |
...
| `wycena.tsx`                            | `#/wycena-kalendarz` (detail panel), `#/wyceny-terenowe`    | **Parity**  | Dodano do panelu szczegółów w `WycenaKalendarz.js`: wyświetlanie sprzętu (rebak/piła/nożyce/kosiarka/podkaszarka/łopata/mulczer/arborysta z pól wyceny), zmiana statusu wyceny (`PATCH /wyceny/:id/status`, role menedżerskie), konwersja na zlecenie (`POST /wyceny/:id/konwertuj`, gdy status=Zaakceptowana). |
...
| `pomocnik.tsx`                          | `AiChat` (globalny, `components/AiChat.js`)                | **Parity**            | Floating AI chat renderowany globalnie w `App.js` — dostępny na każdej stronie; obsługuje kontekst stron i podpowiedzi kontekstowe (PAGE_CONTEXT + SUGGESTED prompts). |
...
7. **Faza G — nawigacja i pomocnik** *(wdrożone: `Eksploruj.js` + `AiChat` globalny)*
  `(tabs)/explore.tsx` → `#/eksploruj` (`Eksploruj.js`): siatka kafelków-skrótów do wszystkich modułów (filtr po roli, wyszukiwarka inline). `pomocnik.tsx` → `AiChat.js` globalny (floating, kontekstowy). Obie trasy w Sidebar (Start).
