...
- Twilio webhook signature validation is conditional on `PUBLIC_BASE_URL`, `TWILIO_AUTH_TOKEN`, and skip flags. Zadarma SMS webhook validation uses the Zadarma webhook signature helper.
...
| SMS/voice webhooks | `/api/sms/webhooks/*`, `/api/telefon/webhooks/*` | Provider -> API -> DB/pipeline | Twilio validation is conditional; Zadarma SMS status uses signature validation | `os/src/routes/sms-webhooks.js`, `os/src/routes/telefon-webhooks.js` |
...
| TM-002 | External webhook spoofer/replayer | Webhook secret missing, weak, leaked, or provider signature validation skipped/misconfigured. | Send forged CRM/Kommo/SMS/voice events to mutate tasks, delivery status, recordings, leads, or attachments. | Integrity compromise and operational disruption. | Tasks, CRM leads, SMS/call logs, recordings | Kommo shared secret fail-closed and timing-safe compare in `kommoQuotationWebhook.js`; Zadarma SMS status signature validation in `sms-webhooks.js`; Twilio signature check in SMS/recording webhooks when config is present. | Twilio validation skipped when base/token absent or skip flag true; generic CRM webhook relies on token path; replay windows are partial/provider-specific. | Fail closed for all provider webhooks in production; require HMAC/signature or high-entropy token; add replay window and idempotency on all mutable webhooks. | Monitor unsigned webhook hits, validation failures, duplicate event keys, provider status changes without known SID/call SID. | Medium | High | High |
