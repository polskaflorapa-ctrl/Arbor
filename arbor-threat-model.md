...
- Twilio webhook signature validation is conditional on `PUBLIC_BASE_URL`, `TWILIO_AUTH_TOKEN`, and skip flags. Zadarma SMS webhook validation uses a dedicated HMAC check for SMS notification/status payloads.
...
| SMS/voice webhooks | `/api/sms/webhooks/*`, `/api/telefon/webhooks/*` | Provider -> API -> DB/pipeline | Twilio validation is conditional; Zadarma SMS status uses dedicated HMAC signature validation | `os/src/routes/sms-webhooks.js`, `os/src/routes/telefon-webhooks.js` |
...
| TM-002 | External webhook spoofer/replayer | Webhook secret missing, weak, leaked, or provider signature validation skipped/misconfigured. | Send forged CRM/Kommo/SMS/voice events to mutate tasks, delivery status, recordings, leads, or attachments. | Integrity compromise and operational disruption. | Tasks, CRM leads, SMS/call logs, recordings | Kommo shared secret fail-closed and timing-safe compare in `kommoQuotationWebhook.js`; Zadarma SMS status HMAC validation in `sms-webhooks.js`/`zadarma.js`; Twilio signature check in SMS/recording webhooks when config is present. | Twilio validation skipped when base/token absent or skip flag true; generic CRM webhook relies on token path; replay windows are partial/provider-specific. | Fail closed for all provider webhooks in production; require HMAC/signature or high-entropy token; add replay window and idempotency on all mutable webhooks. | Monitor unsigned webhook hits, validation failures, duplicate event keys, provider status changes without known SID/call SID. | Medium | High | High |
...
| TM-005 | Remote attacker or webhook-controlled URL | Remote attachment copying enabled or external URLs accepted. | Trigger backend fetch to internal or large remote resources. | SSRF, egress/cost exhaustion, local metadata probing. | Network boundary, storage, secrets, availability | Kommo attachment URL blocks localhost/local names, private/reserved DNS resolutions, private IP literals, and private redirect targets; download has timeout and max bytes in `kommoQuotationWebhook.js`. | DNS rebinding between resolution and connection and content-type allowlisting still need stronger coverage. | Prefer provider-domain allowlist; bind outbound fetch to validated IP or use a controlled proxy; add content-type allowlist where business-safe. | Log outbound attachment host/IP/bytes; alert on blocked private ranges and repeated failures. | Medium | Medium/High | Medium |
